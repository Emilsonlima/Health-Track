				...
				<div class=container>
					...
					<table class="table table-striped">
						...
						<c:forEach items="${atividades}" var="a">
							<tr>
								<td>${a.nm_atividade}</td>
								<td>${a.nr_cal}</td>
								<td><fmt:formatDate value="${a.dt_atividade.time }"
										pattern="dd/MM/yyyy" /></td>
										<td><c:url value="peso" var="link">
										<c:param name="acao" value="abrir-form-edicao" />
										<c:param name="id_atividade" value="${a.id_atividade}" />
									</c:url> <a href="${link}">Editar</a>
									<button type="button" class="btn btn-danger btn-xs"
									data-toggle="modal" data-target="#excluirModal"
									onclick="codigoExcluir.value = ${p.id_atividade}">Excluir</button>
									</td>
							</tr>
						</c:forEach>
					</table>
				...
				<div class="modal-footer">
					<form action="atividade" method="post">
						<input type="hidden" name="acao" value="excluir"> <input
							type="hidden" name="id_atividade" id="codigoExcluir">
